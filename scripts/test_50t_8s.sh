#!/bin/bash
# test_50t_8s.sh
# Test 50 threads with 8 shards (6.25 threads/shard - lower contention)
# Goal: Compare against 4-shard config for 50-thread workload

set -e

# Configuration
CONTAINER_NAME="test-50t-8s"
RESULTS_DIR="results/thread_scale_50t_8s"
TEST_DURATION="10m"
RPS=1000
THREADS=50
BUFFER_SIZE="64MB"
...
# Start
print_header "Thread Scaling: 50 Threads / 8 Shards (6.25 T/S)"
...
echo -e "  - Buffer: ${BUFFER_SIZE}, Shards: ${SHARDS}"
echo -e "  - Thread/Shard Ratio: 6.25 (lower contention, overhead risk)"
echo -e "  - Expected: Good but overhead might reduce benefit vs 4S"
echo ""
...
print_step "Starting server container with 50T / 64MB / 8S..."
...
if ! nc -z localhost 8585 2>/dev/null; then
    print_info "Port not responding yet, waiting 5 more seconds..."
    sleep 5
fi
...
# Final summary
print_header "✓✓✓ 50T / 8S TEST COMPLETE! ✓✓✓"
echo ""
