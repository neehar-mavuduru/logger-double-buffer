...
echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}    Multi-Event Logger Test - Single Process (VM)           ${NC}"
echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
echo ""

# Configuration
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
RESULTS_DIR="results/multi_event_single_process_${TIMESTAMP}"
LOG_DIR="logs"
DURATION="10m"
THREADS=100
LOG_SIZE_KB=300
...
# Calculate thread distribution if not explicitly set
if [ $EVENT1_THREADS -eq 35 ] && [ $EVENT2_THREADS -eq 35 ] && [ $EVENT3_THREADS -eq 30 ]; then
    # Auto-distribute threads based on RPS
    TOTAL_RPS=$((EVENT1_RPS + EVENT2_RPS + EVENT3_RPS))
    if [ $TOTAL_RPS -gt 0 ]; then
...
# Ensure minimum 1 thread per event (if RPS > 0)
if [ $EVENT1_RPS -gt 0 ] && [ $EVENT1_THREADS -lt 1 ]; then EVENT1_THREADS=1; fi
if [ $EVENT2_RPS -gt 0 ] && [ $EVENT2_THREADS -lt 1 ]; then EVENT2_THREADS=1; fi
if [ $EVENT3_RPS -gt 0 ] && [ $EVENT3_THREADS -lt 1 ]; then EVENT3_THREADS=1; fi
...
if [ ! -f "bin/multi_event_test" ]; then
...
echo "  Total RPS: $((EVENT1_RPS + EVENT2_RPS + EVENT3_RPS))"
...
echo -e "${CYAN}Starting multi-event test (single process)...${NC}"
...
./bin/multi_event_test \
    ...
    --event1-name "$EVENT1_NAME" \
    --event1-rps "$EVENT1_RPS" \
    --event1-threads "$EVENT1_THREADS" \
    --event2-name "$EVENT2_NAME" \
    --event2-rps "$EVENT2_RPS" \
    --event2-threads "$EVENT2_THREADS" \
    --event3-name "$EVENT3_NAME" \
    --event3-rps "$EVENT3_RPS" \
    --event3-threads "$EVENT3_THREADS" \
    > "$TEST_LOG" 2>&1 &
TEST_PID=$!
...
════════════════════════════════════════════════════════════
    Multi-Event Logger Test Results (Single Process)
    Timestamp: $(date)
════════════════════════════════════════════════════════════
...
Configuration:
  ...
  Total RPS: $((EVENT1_RPS + EVENT2_RPS + EVENT3_RPS))
...
ROTATED_FILES=$(ls -1 "$LOG_DIR"/event*_*.log 2>/dev/null | wc -l)
