#!/bin/bash
# run_10min_soak_test.sh
# Extended soak test for true writev (10 minutes)
# Tests stability and performance over longer duration

set -e

# Configuration
CONTAINER_NAME="soak-test-10min"
RESULTS_DIR="results/soak_test_10min"
TEST_DURATION="10m"
RPS=1000
THREADS=100
BUFFER_SIZE="128MB"
...
# Start
print_header "10-Minute Soak Test - True Writev Validation"
echo -e "Run started at: $(date)"
echo -e "Results directory: ${RESULTS_DIR}"
echo -e "Test configuration:"
echo -e "  - RPS: ${RPS}"
echo -e "  - Duration: ${TEST_DURATION}"
echo -e "  - Threads: ${THREADS} (baseline config)"
...
# Collect profiles at 2, 4, 6, 8, and 10 minutes
for minute in 2 4 6 8 10; do
    seconds=$((minute * 60))
    ...
    # Allocs profile
    if curl -s http://localhost:6060/debug/pprof/allocs > "$RESULTS_DIR/allocs_${minute}min.prof" 2>/dev/null; then
        print_success "allocs profile saved: allocs_${minute}min.prof"
    fi
