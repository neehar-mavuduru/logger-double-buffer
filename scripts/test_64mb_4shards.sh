#!/bin/bash
# test_64mb_4shards.sh
# Test 64MB buffer with 4 shards (16MB per shard)
# Goal: Lower shard count may reduce overhead but increase contention

set -e

# Configuration
CONTAINER_NAME="test-64mb-4shards"
RESULTS_DIR="results/shard_4_64mb"
TEST_DURATION="10m"
RPS=1000
THREADS=100
BUFFER_SIZE="64MB"
...
# Start
print_header "Shard Test: 64MB Buffer / 4 Shards (16MB each)"
...
echo -e "  - Buffer: ${BUFFER_SIZE}, Shards: ${SHARDS} (16MB per shard)"
echo -e "  - Expected: ~25 threads/shard → Higher contention?"
echo ""
...
print_step "Starting server container with 64MB / 4 shards..."
...
# Final summary
print_header "✓✓✓ 4-SHARD TEST COMPLETE! ✓✓✓"
echo ""
