#!/bin/bash
# test_64mb_16shards.sh
# Test 64MB buffer with 16 shards (4MB per shard)
# Goal: Higher shard count may reduce contention but increase overhead

set -e

# Configuration
CONTAINER_NAME="test-64mb-16shards"
RESULTS_DIR="results/shard_16_64mb"
TEST_DURATION="10m"
RPS=1000
THREADS=100
BUFFER_SIZE="64MB"
...
# Start
print_header "Shard Test: 64MB Buffer / 16 Shards (4MB each)"
...
echo -e "  - Buffer: ${BUFFER_SIZE}, Shards: ${SHARDS} (4MB per shard)"
echo -e "  - Expected: ~6.25 threads/shard → Lower contention?"
echo ""
...
print_step "Starting server container with 64MB / 16 shards..."
...
# Final summary
print_header "✓✓✓ 16-SHARD TEST COMPLETE! ✓✓✓"
echo ""
